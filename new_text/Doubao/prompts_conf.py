...
# 所有信息直接摘录原文，并分为以下四类:
# 1) 文中和图片相关的背景介绍，例如图片中的图例解释和图片相关背景的介绍,例如图片上各个元素代表的意义和图片获取的环境,方法,拍摄的对象等。\
# 2) 只利用了公共知识和可以从图片中获得的信息进行的分析，解释和推理过程,例如图片反映的知识和揭示的规律,可能代表的意义等,可以得出的推断,导致图片现象的原因等。\
# 3) 推理过程中涉及到的知识和定义,例如相关知识,实验方法,实验环境等,专有名词的定义等。

# 如果文中的分析和推理过程，到了其他图片，或者无法从图片中获得的结论，事实，证据等，也无法被现有众所周知的知识印证的结论，证据等，\
# 则不摘录，以无代替。
# 所有的信息请整理出JSON格式, key分别为:
# "imageDescription", "analysisResults","relatedKnowledge"
...
# {markdown_content}
# 注意第三和第四点所摘录的内容需要只基于次张图像的内容进行的分析，解释和推理过程,例如图片反映的知识和揭示的规律,可能代表的意义等,可以得出的推断,导致图片现象的原因等。
# 如果出现了其他图片的信息，和需要结合文中其他结论，事实，证据等，或者无法从图片中获得的结论，事实，证据等，
# 则不摘录，以无代替。
# """,
33: """
给定该图片和相关的上下文，我需要你从文中摘录所有和该图片相关的信息, 所有信息用于多模态模型的训练数据。
