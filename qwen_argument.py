from Qwen.dataargument import get_total_responses,check_data_quality
import asyncio
import json
import pandas as pd
import argparse
...
if __name__ == "__main__":
    ...
    # index argument could be an integer or a float, default is 34.2
    parser.add_argument("--index", type=int, default=343, help="index for the qa generation task")
    parser.add_argument("--file_path", type=str, default="/mnt/workspace/MLLM/zc/tclreasoning/data/100TestOutputt/total_response.json", help="path to the input json file")
    parser.add_argument("--image_folder", type=str, default="/mnt/workspace/MLLM/zc/tclreasoning/data/100Test", help="path to the image folder")
    parser.add_argument("--pool_size", type=int, default=100, help="number of parallel tasks")
    parser.add_argument("--output_file", type=str, default="/mnt/workspace/MLLM/zc/tclreasoning/data/100TestOutputt", help="path to the output json file")
    ...
    if not args.check_task:
        ...
        results = asyncio.run(get_total_responses(index, file_path, image_folder, pool_size,stream = stream))
        ...
        asyncio.run(check_data_quality(ark_url, api_key, model, output_file, check_indexes, pool_size=pool_size, check_times=check_times,stream = args.user_stream))