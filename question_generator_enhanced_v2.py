"""
问题生成器 - WebSailor核心模块（优化版V2）
基于子图中节点与关系，设计QA问题
覆盖多种问题类型：事实型、比较型、推理型、多跳型、开放型等
增加了问题合理性验证和优化机制
新增：智能生成策略、质量评估、动态调整等功能
"""
import uuid
import json
import logging
...
class QuestionGenerator:
    ...
    def __init__(self, config: dict):
        ...
        self.question_types = self.qg_config.get('question_types', [
            'factual', 'comparison', 'reasoning', 'multi_hop', 'open_ended',
            'analytical', 'descriptive', 'causal', 'hypothetical'
        ])
        ...
        self.question_type_priorities = {
            ...
            'comparative': 4,       # 比较性问题 - 中等优先级
            'inferential': 1,       # 推理性问题 - 最高优先级
            'open_ended': 2,        # 开放性问题 - 高优先级
            'descriptive': 3,       # 描述性问题 - 中等优先级
            'causal': 4,           # 因果关系问题 - 中等优先级
            'hypothetical': 4,      # 假设性问题 - 较低优先级
            'reasoning': 1,         # 推理型问题 - 最高优先级
            'multi_hop': 3,         # 多跳型问题 - 中等优先级
            'comparison': 4         # 比较型问题 - 中等优先级
        }
        ...
        logger.info("初始化增强版问题生成器V2，支持智能生成策略和质量评估")
    ...
    def _load_qa_generator(self):
        ...
        try:
            ...
            logger.warning(f"无法加载指定模型，使用默认配置: {e}")
            # 降级处理
    ...
    def _init_question_templates(self):
        ...
        self.question_templates = {
            ...
            'comparison': [
                ...
                "{entity1}与{entity2}哪个更{criteria}？"
            ],
            'reasoning': [
                "为什么{entity}会{action}？",
                "如何解释{entity}的{property}？",
                "基于什么原因{entity}具有{characteristic}？",
                "从逻辑上分析，{entity}为何{behavior}？",
                "可以推断出{entity}的什么特点？"
            ],
            'multi_hop': [
            ...
            'causal': [
                ...
                "什么因素造成了{entity}的{state}？",
                "{entity}的根本原因是什么？",
                "解释{entity1}与{entity2}的因果关系。",
                "{entity}产生的后果是什么？"
            ],
            'hypothetical': [
                "假如{entity}具有{property}，会怎样？",
                "设想{entity}在{scenario}中的表现。",
                "如果改变{entity}的{aspect}，结果如何？",
                "想象{entity}的理想状态是什么？",
                "假设{entity}不受{constraint}限制，会如何？"
            ]
        }
    ...
    def _select_question_type(self, type_distribution: Dict[str, int], 
        ...
        for q_type, target in type_distribution.items():
            ...
            if current < target:
                ...
                needed_types.extend([q_type] * (target - current) * (6 - priority))
        ...
        try:
            ...
            if qa_pair:
                ...
                qa_pair.update({
                    'id': str(uuid.uuid4()),
                    'type': question_type,
                    'generation_method': 'template' if random.random() < self.generation_strategy['template_ratio'] else 'llm',
        ...
        try:
            ...
            if '{entity}' in template:
                ...
                if len(entities) >= 2:
                    ...
                    answer = f"关于{entity1}和{entity2}的比较回答。"
                else:
                    return None
            else:
                return None
